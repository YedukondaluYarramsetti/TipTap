...
class PaymentSuccessViewController: UIViewController {
    ...
    @IBAction func doneButton(_ sender: Any) {
        let controller = self.storyboard?.instantiateViewController(identifier: "TipsViewController")as! TipsViewController
        controller.modalPresentationStyle = .fullScreen
        controller.modalTransitionStyle = .flipHorizontal
        controller.tipsImage = personImage ?? ""
        self.present(controller, animated: true)
    }
...
/*
 The Core Animation framework is a graphics rendering and animation framework provided by Apple, designed to create smooth, high-performance animations and visual effects in applications across iOS, macOS, watchOS, and tvOS. It allows developers to animate properties of visual elements, such as position, size, opacity, and transformations, by manipulating lightweight objects called layers (CALayer). Core Animation handles the rendering and compositing of these layers efficiently, often offloading work to the GPU to maintain high frame rates and ensure a responsive user interface.
